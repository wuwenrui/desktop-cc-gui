...
describe("messagesTimelineVirtualization", () => {
  ...
  describe("shouldVirtualizeTimelineRows", () => {
    it("transitions from static to virtualized when streaming completes on a long timeline", () => {
      const rowCount = TIMELINE_VIRTUALIZATION_MIN_ROWS + 50;
      expect(shouldVirtualizeTimelineRows({ isThinking: true, rowCount })).toBe(false);
      expect(shouldVirtualizeTimelineRows({ isThinking: false, rowCount })).toBe(true);
    });
  });
});
