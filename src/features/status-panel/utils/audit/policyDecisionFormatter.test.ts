...
describe("formatPolicyDecision", () => {
  ...
  it("falls back when source id is missing or blank", () => {
    expect(
      formatPolicyDecision(decision({ sourceId: null }), t).sourceLabel,
    ).toBe("statusPanel.audit.sourceUnavailable");
    expect(
      formatPolicyDecision(decision({ sourceId: "   " }), t).hasSource,
    ).toBe(false);
  });
  ...
  it("handles no_contribution without inventing evidence", () => {
    ...
    expect(result.verdictLabel).toBe(
      "statusPanel.policy.verdict.no_contribution",
    );
    expect(result.enforcementLabel).toBe(
      "statusPanel.audit.enforcement.informational",
    );
  ...
  it("keeps unknown policy ids as labels", () => {
    const result = formatPolicyDecision(
      decision({ policyId: "unknownPolicy" }),
      t,
    );
  ...
  it("formats bridge-fed audit metadata defensively", () => {
    ...
    expect(result.evidenceSnapshotTitle).toBe("snapshot-1");
    expect(result.degradationLabel).toBe("pricing-unavailable");
    expect(result.staleLabel).toBe("2026-05-20T00:00:00.000Z");
  });

  it("compacts long evidence snapshot ids for audit rows", () => {
    const longSnapshotId = `large-file:long-artifact:${JSON.stringify({
      artifactPath:
    ...
    const result = formatPolicyDecision(
      decision({
        evidenceSnapshotId: longSnapshotId,
      }),
      t,
    );

    expect(result.evidenceSnapshotTitle).toBe(longSnapshotId);
    expect(result.evidenceSnapshotLabel).not.toBe(longSnapshotId);
    expect(result.evidenceSnapshotLabel).toContain("…");
  });
});
