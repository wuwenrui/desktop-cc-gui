...
修复 assistant item/updated 快照缺少 terminal 事件时前端仍显示生成中的问题；新增复现回归测试并完成前端、Rust 门禁。
...
为侧栏普通/置顶会话新增 Codex 风格 hover preview，并完成全量前端验证。

### Main Changes

目标：把 Codex app 左侧会话 hover preview 设计落到桌面端侧栏。

主要改动：
- 新增 ThreadHoverPreviewCard 共享组件。
- ThreadList 与 PinnedThreadList 支持 hover/focus 受控预览卡。
- 补充中英文 i18n 与 sidebar 样式，预览展示标题、状态、引擎、更新时间、工作区路径。
- 新增普通会话与置顶会话 preview 回归测试。
- 新增 OpenSpec change add-thread-hover-preview，记录 proposal/design/tasks/spec delta/verification。

验证：
- npx vitest run src/features/app/components/ThreadList.test.tsx src/features/app/components/PinnedThreadList.test.tsx：33 tests passed。
- npx vitest run src/features/threads/hooks/useThreads.pin.integration.test.tsx src/features/threads/hooks/useThreads.integration.test.tsx src/features/threads/hooks/useThreads.memory-race.integration.test.tsx：43 tests passed。
- npm run test：770 test files completed。
- npm run typecheck：passed。
- npm run lint：0 errors，保留既有 Composer exhaustive-deps warning。
- git diff --check：passed。

限制：
- openspec validate 未执行，本机 openspec CLI 与上游 validator 缺失；已写入 openspec/changes/add-thread-hover-preview/verification.md。
...
## Session 13: 恢复消息锚点导航条

**Date**: 2026-07-02
**Task**: 恢复左缘消息锚点导航与长回复大纲浮层
**Branch**: `main`

### Summary

上游 f763bbf7 删除了 MessagesAnchorRail/MessagesOutlineFloater 接线并随合并进入 fork，用户端丢失消息导航；本次 revert 恢复并剔除该提交捆绑的无关回退。

### Main Changes

- revert f763bbf7：恢复 MessagesAnchorRail 组件、Messages 的 RAF 活跃锚点追踪、MessagesTimeline 大纲浮层接线与锚点回归测试。
- 冲突消解保留 HEAD 现状：explore-inline 视觉改版、ToolMarkerShell 展开箭头不回退；MessagesRows.tsx 与 messages.part1.css 与恢复前字节一致。
- 样式（messages.status-shell.css / messages-outline-floater.css）与 i18n（locales part1 anchorNavigation/anchorUserTitle）在 fork 拆分文件中原本未删，直接复用。
- 新增 OpenSpec change restore-message-anchor-rail（proposal/tasks/spec delta/verification）。

验证：
- npx vitest run src/features/messages/：68 test files，572 passed / 7 skipped，含恢复的 "renders user-only anchors and scrolls on click"。
- npm run test：770 test files completed，exit 0。
- npm run typecheck：passed。
- npx eslint 四个改动组件：0 errors。

限制：
- openspec validate 未执行（本机无 openspec CLI）。
- 后续上游合并需警惕该删除被再次带入。
...
## Session 14: 无进展怀疑升级强制收尾

**Date**: 2026-07-02
**Task**: 修复 v0.6.5 仍复现的忙碌态无限悬挂
**Branch**: `main`

### Summary

用户机 v0.6.5 复现"正在生成响应"转圈 12 分钟以上；定位到两个防御死角并补齐：codex 怀疑态永不收尾、claude 看门狗被僵尸执行项无限改期。

### Main Changes

- codex：markCodexNoProgressSuspected 后挂 120s 升级定时器（CODEX_NO_PROGRESS_FORCE_SETTLEMENT_MS），到期仍无进展且 deltaCount>0 则走 applyAssistantFinalSettlementFallback 强制收尾；进展恢复取消升级；collab 阻塞时重试。
- claude：静默看门狗活跃执行项加 TURN_INGRESS_EXECUTION_STALE_CEILING_MS（10 分钟）上限，超龄执行项不再改期。
- 升级定时器清理覆盖：进展恢复、收尾、重排（clearCodexNoProgressTimer 内联）、unmount。
- 新增回归测试 3 项；OpenSpec change settle-stalled-turn-busy-state。

验证：
- npx vitest run useThreadEventHandlers.test.ts：68 passed。
- npx vitest run src/features/threads/：119 files，1359 passed。
- npm run test：770 files completed（发现 LawhubNavSection 一例并发 flaky，隔离通过，与本次无关；批测 runner 失败不传导 exit code，已另挂修复任务）。
- npm run typecheck：passed；eslint 改动文件 0 errors。

### Git Commits

| Hash | Message |
|------|---------|
| `0824dd1d` | (see git log) |
...
- 发布后由用户回归同场景（deepseek-v4-pro 长回合流中断）确认不再无限转圈
