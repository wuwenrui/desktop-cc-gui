# windows-offline-installer Specification

## Purpose
Defines the Windows NSIS installer WebView2 offline install strategy for `ccgui`, including `offlineInstaller` mode, Tauri-managed Evergreen Standalone Installer bundling, platform isolation from macOS/Linux bundles, fall-through behavior when a compatible WebView2 runtime already exists, and the prohibition on silently regressing to Tauri's default `downloadBootstrapper` path.
## Requirements
