...
- **Successors**:
  - Step 2 `renderer-resource-backpressure` 会拆 `app-shell.tsx` listener owner，**本 change 必须先把 Composer 状态从 shell 订阅里解耦**，否则 Step 2 改 `app-shell.tsx` 时会与本 change 撞在同一段。
...
- **Blocking Rule**: 本 change 不通过 `openspec validate`、Composer / message row evidence gate 未落地前，Step 2 不应启动。
