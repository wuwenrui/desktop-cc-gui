# Spec Delta: runtime-perf-evidence-classification

## Purpose

定义 runtime performance evidence 的 `proxy` / `synthetic` / `measured` 三档分类,以及 `proxyRatio` warn 规则,使 v0.5.11 阶段 81% proxy 数据能在本轮逐步升级到 measured 主导,避免 release 报告只放 proxy 数据。
