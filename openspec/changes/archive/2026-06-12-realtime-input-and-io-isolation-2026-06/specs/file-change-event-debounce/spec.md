...
> **Implementation Status (本 change)**: Rust emitter (`DebouncedExternalChangeEmitter`) 已完成,含 4 个 inline test;frontend follow-up 已完成 `useFileExternalSync` 互斥 batch consumer + batch 入口 `(workspaceId, normalizedPath)` latest-wins coalesce,并复用既有 in-flight / stale-drop path。
