...
> **Implementation Status (本 change)**: 本 change 已完成 (file command surface 经 `run_blocking_file_io` 包装,含本轮校准补全的 `paste_workspace_item` / `rename_workspace_item` / `write_external_absolute_file` / `paste_external_workspace_items_core`)。
