...
- [ ] 10.6 [P1][depends:10.5][input: 手动验证步骤(在 `tauri dev` 下)][output: 启动 runtime 后点击顶栏 / 右侧工具栏任一 icon 按钮,click → 视觉反馈 < 100ms;切右侧 panel tab,popover 关闭 + 新 tab 高亮 < 150ms;记录 manual 验证结果到 `verification.md`][validation: verification.md 有手动验证时间戳 + 截图或描述] Manual verification.
