...
func TestRewriteReplyWithImageResults(t *testing.T) {
	reply := "已生成图片\n![image](https://example.com/out.png)\n![image](https://example.com/missing.jpg)"

	got := rewriteReplyWithImageResults(
		reply,
		[]string{"https://example.com/out.png"},
		[]string{"https://example.com/missing.jpg"},
	)

	if strings.Contains(got, "![image]") {
		t.Fatalf("expected markdown image syntax to be removed, got %q", got)
	}
	if !strings.Contains(got, "已发送图片：out.png") {
		t.Fatalf("expected sent image note, got %q", got)
	}
	if !strings.Contains(got, "图片发送失败：missing.jpg") {
		t.Fatalf("expected failed image note, got %q", got)
	}
}
